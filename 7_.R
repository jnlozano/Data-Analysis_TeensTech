...
if(originalx == 1){
medians$percentage <- ((medians$x-medians[13,2])/abs(medians[13,2])*100) #JL:number here changes reference variable. 13 is default 
medians$times <- medians$x/medians[13,2]
}else if(originalx == 0){
  medians$percentage <- ((medians$x-medians[16,2])/abs(medians[16,2])*100) #JL:number here changes reference variable. 13 is default 
  medians$times <- medians$x/medians[16,2]}
#JL: change 13 for reference variable ie tech. Make tech last one  

# JL: Add sex column and make times absolute values for medians.  
if(sex == 0){
  medians$sex <- "girl"
}else if(sex == 1){
  medians$sex <- "boy"
...
while(i<16){
  ...
  if(is.positive(medians$times[i]) == FALSE){
    medians$times[i] <- abs(medians$times[i])}
  i=i+1}
if(originalx ==1){
  # JL: change order of variables to match Orben
medians <- medians[c(1, 4, 9, 2, 5, 8, 7, 11, 6, 10, 12, 3, 13),]
}
medians

#write.csv(medians, "7_combined_medians_mcs.csv")
write.table(medians, "7_comp_medians_mcs.csv", sep = ",", col.names = !file.exists("7_comp_mcs.csv"), append = T)
...
if(originalx ==1){
  number<- number[c(1, 4, 9, 2, 5, 8, 7, 11, 6, 10, 12, 3, 13),]
}
number

#write.table(number, "7_comp_numbers_mcs.csv", sep = ",", col.names = !file.exists("7_comp_mcs.csv"), append = T)
write.table(number, "7_comp_numbers_limited_mcs.csv", sep = ",", col.names = !file.exists("7_comp_mcs.csv"), append = T)
