...
data <-
  read.spss(
    paste("YouthRiskCDC91-15CPX.sav", sep = ""),
...
#######################################################
data <- data %>% filter(year >= 2013) #ONLY 2013+ DATA USED 
...
#Jean's participant variable#
i = 1
while (i < length(data$q27_n)){
  if (is.na(data$q27_n[i]==FALSE) || data$q27_n[i] == 0 || is.na(data$q28_n[i]==FALSE) || data$q28_n[i] == 0  || is.na(data$q29_nd[i]==FALSE) || is.na(data$q30_nd[i]==FALSE) || data$q30_nd[i] == 0){
    data$suic[i] <- 0 
  }
  else {
    data$suic[i] <- 1 
